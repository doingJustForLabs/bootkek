...
class ChatRepository:
    ...
    @staticmethod
    async def create_chat(session: AsyncSession, name: str, user_ids: List[int]) -> Chat:
    ...
    async def get_chat_by_id(session: AsyncSession, chat_id: int) -> Optional[Chat]:
        result = await session.execute(select(Chat).filter(Chat.id == chat_id).options(selectinload(Chat.users)))
        return result.scalars().first()

    @staticmethod
    async def get_chats_for_user(session: AsyncSession, user_id: int) -> List[Chat]:
        result = await session.execute(
            select(Chat).join(ChatUser).filter(ChatUser.user_id == user_id).options(selectinload(Chat.users))
        )
    ...
    @staticmethod
    async def add_message_to_chat(session: AsyncSession, chat_id: int, user_id: int, content: str) -> Message:
