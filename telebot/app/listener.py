...
async def pg_listener(bot: Bot):
    ...
    async def listen():
        await conn.add_listener("new_follower", lambda *args: asyncio.create_task(handle_new_follower(bot, args)))
...
async def handle_new_follower(bot: Bot, args):
    ...
    async with pool.acquire() as conn:
        ...
        target = await conn.fetchrow("SELECT tg_id FROM profiles WHERE user_id = $1", target_id)
        print("target:", target)
        # Получаем имя подписчика
        follower = await conn.fetchrow("SELECT name FROM profiles WHERE user_id = $1", follower_id)
